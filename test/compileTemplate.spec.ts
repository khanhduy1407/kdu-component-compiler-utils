...
import * as compiler from 'kdu-template-compiler'
import Kdu from 'kdu'

import { KduTemplateCompiler } from '../lib/types'
...
function mockRender(code: string, options: any = {}) {
  ...
  const vm = new Kdu(Object.assign({}, options))
  vm.$mount()
...
test('should work', () => {
  ...
  const result = compileTemplate({
    filename: 'example.kdu',
    source,
    compiler: compiler as KduTemplateCompiler
  })
...
/**
 * khanhduy1407/kdu-component-compiler-utils Support uri fragment in transformed require
 */
